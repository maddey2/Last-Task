...
- (void)viewDidLoad {
    ...
    UISwipeGestureRecognizer *swipeGestureLeft = [[UISwipeGestureRecognizer alloc]initWithTarget:self action:@selector(calRight:)];
    swipeGestureLeft.direction = UISwipeGestureRecognizerDirectionLeft;
    swipeGestureLeft.delegate = self;
    [self.view addGestureRecognizer:swipeGestureLeft];
    
    UISwipeGestureRecognizer *swipeGestureRight = [[UISwipeGestureRecognizer alloc]initWithTarget:self action:@selector(calLeft:)];
...
-(void)viewDidAppear:(BOOL)animated
{
    ...
    viewChart = [[UUChart alloc]initwithUUChartDataFrame:_viewGraph.bounds withSource:self withStyle:UUChartBarStyle];
    
    viewChart.frame = CGRectMake(viewChart.frame.origin.x, viewChart.frame.origin.y, viewChart.frame.size.width, viewChart.frame.size.height-8);
    
    viewChart.layer.borderColor = [[UIColor lightGrayColor] CGColor];
    viewChart.layer.borderWidth = 1.0;
    
    [viewChart showInView:_viewGraph];
    ...
    switch ((int)[UIScreen mainScreen].bounds.size.height) {
        ...
        case 667:
        {
            _lblXConstraint.constant = _lblXConstraint.constant+4.0;
            ...
            _lblFlext.font = [UIFont fontWithName:@"Roboto-Bold" size:25];
            _lblEconomy.font = [UIFont fontWithName:@"Roboto-Bold" size:25];
            _lblCost1.font = [UIFont fontWithName:@"Roboto-Bold" size:25];
            _lblCost2.font = [UIFont fontWithName:@"Roboto-Bold" size:25];
        ...
        case 736:
        {
            _lblXConstraint.constant = _lblXConstraint.constant+8.0;
...
-(BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch
{
    ...
    if([view isKindOfClass:[UUBarChart class]])
    {
        return YES;
    }
...
-(void)dateChange
{
    NSArray *labelArray = @[_label1,_label2,_label3,_label4,_label5,_label6,_label7];
    ...
    for (NSInteger i = 0; i < 7; ++i) {
        ...
        if (i==6) {
            ...
            _dateLabel.text = [[dateFormatter stringFromDate:nextDate] capitalizedString];
...
- (IBAction)calRight:(id)sender {
    ...
    /////
    if(_rightBtn.userInteractionEnabled)
    {
    NSCalendar *calendar = [NSCalendar currentCalendar];
